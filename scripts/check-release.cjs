...
// be replaced, only deprecated. Every check below is a way a release here has
// actually gone wrong, so each one reports the specific fix rather than just
// refusing.
...
// `git checkout v4.7.1 && make publish` detaches HEAD. The old publish recipe
// then pushed `HEAD:master`, which tried to move master *backwards* onto the tag
// and was rejected as a non-fast-forward.
...
if (tagged !== head) {
  fail(
    ...
    `you committed after tagging. Since ${tag} is not on npm yet, move it: ` +
...
if (remote !== head) {
  try {
    execFileSync('git', ['merge-base', '--is-ancestor', remote, head], {cwd: root});
  } catch {
    fail(
