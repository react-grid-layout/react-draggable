...
export default class DraggableCore extends React.Component<DraggableCoreProps> {
    ...
    handleDragStart: EventHandler<MouseTouchEvent> = (e) => {
        ...
        if (
            ...
            const onContextMenu = (contextEvent: MouseTouchEvent) => {
                contextEvent.preventDefault();
            };
            addEvent(ownerDocument, 'contextmenu', onContextMenu);

            const onTouchEnd = () => {
                log('DraggableCore: touchend detected, canceling drag timeout');
                clearTimeout(this.dragTimeout);
                removeEvent(ownerDocument, eventsFor.touch.stop, onTouchEnd);
                removeEvent(ownerDocument, 'contextmenu', onContextMenu);
            };
    ...
    onContextMenu: EventHandler<MouseTouchEvent> = (e) => {
