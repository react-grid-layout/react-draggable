...
export function createDraggableData(draggable: Draggable, coreData: DraggableData): DraggableData {
  const { scale, angle } = draggable.props;
  const { deltaX, deltaY, node } = coreData;
  const { x: lastX, y: lastY } = draggable.state;
  ...
  if (angle === 0) {
    ...
    return {
      node,
      x: lastX + scaledDeltaX,
      y: lastY + scaledDeltaY,
      deltaX: scaledDeltaX,
      deltaY: scaledDeltaY,
      lastX,
      lastY,
  ...
  const scaledDeltaX = deltaX / scale;
  const scaledDeltaY = deltaY / scale;
  const deltaXTransformed = scaledDeltaX * cos + scaledDeltaY * sin;
  const deltaYTransformed = -scaledDeltaX * sin + scaledDeltaY * cos;

  return {
    node,
    x: lastX + deltaXTransformed,
    y: lastY + deltaYTransformed,
    deltaX: deltaXTransformed,
    deltaY: deltaYTransformed,
    lastX,
    lastY,
  };
}
