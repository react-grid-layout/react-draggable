...
export function snapToGrid(grid: [number, number], scale: number, pendingX: number, pendingY: number): [number, number] {
  const x = Math.round(pendingX / (grid[0] * scale)) * grid[0];
  const y = Math.round(pendingY / (grid[1] * scale)) * grid[1];
  return [x, y];
}
