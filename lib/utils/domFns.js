...
export function createCSSTransform({x, y}: {x: number, y: number}, defaultPosition: DefaultControlPosition): Object {
  let translation;
...
export function createSVGTransform({x, y}: {x: number, y: number}, defaultPosition: DefaultControlPosition): string {
  let translation;
  if (defaultPosition && (defaultPosition.x !== 0 || defaultPosition.y !== 0)) {
