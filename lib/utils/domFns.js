...
import type {ControlPosition, InitialControlPosition, MouseTouchEvent} from './types';
...
export function createCSSTransform({x, y}: ControlPosition, initialPosition: InitialControlPosition): Object {
  let translation;
  if (initialPosition && (initialPosition.x !== 0 || initialPosition.y !== 0)) {
    const defaultX = `${(typeof initialPosition.x === 'string') ? initialPosition.x : initialPosition.x + 'px'}`;
    const defaultY = `${(typeof initialPosition.y === 'string') ? initialPosition.y : initialPosition.y + 'px'}`;
...
export function createSVGTransform({x, y}: ControlPosition, initialPosition: InitialControlPosition): string {
  let translation;
  if (initialPosition && (initialPosition.x !== 0 || initialPosition.y !== 0)) {
    const defaultX = (typeof initialPosition.x === 'string') ? initialPosition.x : initialPosition.x;
    const defaultY = (typeof initialPosition.y === 'string') ? initialPosition.y : initialPosition.y;
