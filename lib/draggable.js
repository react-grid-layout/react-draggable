...
function calculateAfterRotate( orginalW,originalH,radians) {
    var x1 = -orginalW/2,
        x2 = orginalW/2,
        x3 = orginalW/2,
        x4 = -orginalW/2,
    ...
    return [Math.ceil(x_max-x_min),Math.ceil(y_max-y_min)];
}
...
function getBoundPosition(draggable, clientX, clientY) {
  ...
  if (bounds === 'parent') {
    ...
    var customTransform = draggable.props.children.props.style.transform ? draggable.props.children.props.style.transform : '';
    var transformArray = customTransform.split(' ');

    var elementWidth = outerWidth(node);
    var elementHeight = outerHeight(node);
    var moveX = 0
    var moveY = 0

    if (transformArray.length > 0) {
      var rotate = 0;
      var rotatePattern = /rotate\(?(.*?)deg\)/

      for (var i = transformArray.length - 1; i >= 0; i--) {
        var matchesRotate = rotatePattern.exec(transformArray[i]);
        if (matchesRotate && matchesRotate[1]) {
          rotate = parseFloat(matchesRotate[1])
        }
      }
      if (rotate !== 0) {
        var rotated = calculateAfterRotate(elementWidth, elementHeight, rotate* (Math.PI/180))
        moveX = (elementWidth - rotated[0])/2;
        moveY = (elementHeight - rotated[1])/2;
        elementWidth = rotated[0];
        elementHeight = rotated[1];
      }
    }
    ...
    bounds = {
      left: -node.offsetLeft + int(parentStyle.paddingLeft) +
            int(nodeStyle.borderLeftWidth) + int(nodeStyle.marginLeft) - moveX,
      top: -node.offsetTop + int(parentStyle.paddingTop) +
            int(nodeStyle.borderTopWidth) + int(nodeStyle.marginTop) - moveY,
      right: innerWidth(parent) - elementWidth - node.offsetLeft - moveX,
      bottom: innerHeight(parent) - elementHeight - node.offsetTop - moveY
    };
  }
...
function createCSSTransform(style) {
  ...
  var x = style.x + 'px';
  var y = style.y + 'px';
  var transform = style.childTransform;
  var out = {transform: 'translate(' + x + ',' + y + ')'+ transform};
...
module.exports = React.createClass({
  ...
  render: function () {
    ...
    var transform = createCSSTransform({
      ...
      // Extends by child transform styles
      childTransform: childStyle.transform ?
