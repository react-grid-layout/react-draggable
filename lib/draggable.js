...
/*function isThisSVG(element) {
  
  // Container to temporarily render the node
  var container = document.createElement('div');
  container.style.display = 'none';
  document.body.appendChild(container);

  // Give us a way to find the rendered DOMNode
  var taggedElement = React.cloneElement(element, {id: "svgTest"});

  // Render the DOMNode()
  React.render(taggedElement, container);

  // Find the rendered element
  var elementDOM = document.getElementById("svgTest");

  // Store output of test
  var isSVG = elementDOM instanceof SVGElement;

  // Cleanup
  React.unmountComponentAtNode(container, function() { document.removeElement(container) });
