...
const log = typeof process !== "undefined" && false ? 0 : function noop() {
};
...
class DraggableCore extends external_commonjs_react_commonjs2_react_amd_react_root_React_.Component {
  constructor() {
    ...
    this.handleDragStart = (e) => {
      ...
      utils_log("DraggableCore: handleDragStart: %j", coreEvent);
      utils_log("calling", this.props.onStart);
    ...
    this.handleDrag = (e) => {
      ...
      const coreEvent = createCoreData(this, x, y);
      utils_log("DraggableCore: handleDrag: %j", coreEvent);
    ...
    this.handleDragStop = (e) => {
      ...
      utils_log("DraggableCore: handleDragStop: %j", coreEvent);
      this.dragging = false;
      this.lastX = NaN;
      this.lastY = NaN;
      if (thisNode) {
        utils_log("DraggableCore: Removing handlers");
...
class Draggable extends external_commonjs_react_commonjs2_react_amd_react_root_React_.Component {
  constructor(props) {
    ...
    this.onDragStart = (e, coreData) => {
      utils_log("Draggable: onDragStart: %j", coreData);
    ...
    this.onDrag = (e, coreData) => {
      ...
      utils_log("Draggable: onDrag: %j", coreData);
    ...
    this.onDragStop = (e, coreData) => {
      ...
      utils_log("Draggable: onDragStop: %j", coreData);
      const newState = {
        dragging: false,
        slackX: 0,
        slackY: 0
  ...
  static getDerivedStateFromProps({ position }, { prevPropsPosition }) {
    if (position && (!prevPropsPosition || position.x !== prevPropsPosition.x || position.y !== prevPropsPosition.y)) {
      utils_log("Draggable: getDerivedStateFromProps %j", { position, prevPropsPosition });
      return {
        x: position.x,
        y: position.y,
...
/******/ 	let __webpack_exports__ = __webpack_require__(616);
/******/ 	
